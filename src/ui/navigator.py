...
class AnalyzeSelector:
    ...
    def render(self):
        ...
        if available > 40:
            bar_w = 20
            name_w = min(40, available - bar_w)
        elif available > 20:
            bar_w = 10
            name_w = available - bar_w
        else:
            bar_w = 0
            name_w = max(15, available)
        print(f"{hint}\n")
        ...
        try:
            while True:
                ...
                if key == Navigator.UP:
                    ...
                    self.selected_index = (self.selected_index + 1) % len(self.items)
                elif key in (Navigator.LEFT, "b", "B", "h", "H"):
                    return "BACK", None
...
class PaginatedSelector:
    ...
    def render(self):
        ...
        for i in range(start, end):
            ...
            print(
                f"{cursor} {checkbox} {style}{item['human_size']:>8} | {item['project']:<15} | {item['path'].name}{RESET}"
            )
        print("-" * 60)
        total_pages = (len(self.items) + self.page_size - 1) // self.page_size
        print(
            f" Page {self.current_page + 1}/{total_pages} | Items: {len(self.items)} | Selected: {len(self.selected_items)}"
        )
        print(
            f"{GRAY} ↑/↓: Move | Space: Select | A: All | P: Purge | M: Edit Paths | ESC: Back{RESET}"
        )
    ...
    def render(self):
        ...
        if total_len == 0:
            ...
            for i in range(start, end):
                ...
                print(
                    f"{cursor} {checkbox} {name_style}{name_padded}{RESET}     {item['size_str']:>12} | {time_str}"
                )
            print("-" * 80)
            order_icon = "↓" if self.sort_reverse else "↑"
...
class TopFilesSelector:
    ...
    def render(self):
        ...
        fixed_overhead = 2 + 4 + 12 + 5 + 3
        name_w = columns - fixed_overhead
        for i in range(start_idx, end_idx):
            ...
            print(
                f"{cursor} {checkbox} {WHITE}{bytes_to_human(item['size_bytes']):>12}{RESET}{age_str} | {style}{display_path}{RESET}"
            )
        print("-" * (columns - 2))
        ...
        if self.selected_items:
            ...
            for i in sorted(list(self.selected_items)):
                item = self.items[i]
                print(f"   \033[1;35m•\033[0m 📄 {Path(item['path']).name}")
