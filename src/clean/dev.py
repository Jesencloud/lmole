...
from ..core.file_ops import (
    safe_remove, get_size, bytes_to_human, clean_path_by_age, 
...
def clean_tool_cache(description, command_args, cache_path=None, dry_run=False):
    ...
    if cache_path:
        ...
        if path.exists(): total_size = get_size(path)
...
def clean_docker(dry_run=False):
    ...
    if shutil.which("docker"):
        ...
        use_sudo = True
        try:
            if subprocess.run(["docker", "info"], capture_output=True).returncode == 0: use_sudo = False
        except: pass
...
def clean_podman(dry_run=False):
    ...
    total_size = 0; items = 0
    if shutil.which("podman"):
        if dry_run:
            print("  \033[0;32m✓\033[0m Podman (unused images/volumes) would be pruned"); items += 1
        else:
            run_command(["podman", "system", "prune", "-f"], capture=True)
            print("  \033[0;32m✓\033[0m Podman system pruned"); items += 1
        ...
        if cache_path.exists():
            ...
            total_size += s; items += i
            if i > 0 and not dry_run: print(f"  \033[0;32m✓\033[0m Podman transfer cache ({bytes_to_human(s)}) cleaned")
...
def clean_developer_tools(dry_run=False):
    ...
    if cargo_path.exists():
        register_cleaned_path(cargo_path); size = get_size(cargo_path)
        if size > 1024:
            if dry_run: print(f"  \033[0;32m✓\033[0m Cargo cache ({bytes_to_human(size)}) would be cleaned")
            else: shutil.rmtree(cargo_path, ignore_errors=True); print(f"  \033[0;32m✓\033[0m Cargo cache ({bytes_to_human(size)}) cleaned")
            total_size += size; total_items += 1; total_categories += 1
